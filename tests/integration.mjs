...
async function test() {
    ...
    try {
        ...
        await driver.findElement({ className: "start-tests-button" }).click();

        console.log("Waiting for tests to finish");
        await driver.wait(async () => {
            const result = await driver.findElement({ id: "result-number" }).getText();
            return result !== "";
        });
        const result = await driver.executeScript(() => {
            return window.benchmarkClient._measuredValuesList[0];
        });

        console.log(`---- Results for ${BROWSER} ----`);
